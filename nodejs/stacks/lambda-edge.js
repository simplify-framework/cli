    ...
    provider.setConfig(config).then(function () {
        simplify.uploadLocalFile({
            ...
            function processStackData(stackData) {
                ...
                stackData.Outputs.map(function (o) {
                    if (o.OutputKey == `CFDistribution`) {
                        outputData.CFDistribution = o.OutputValue
                    } else if (o.OutputKey == `Region`) {
                        outputData.CFDistributionRegion = o.OutputValue
                    } else if (o.OutputKey == `StackId`) {
                        outputData.CFDistributionStackId = o.OutputValue
                    }
                })
            ...
            simplify.createOrUpdateStackOnComplete({
                ...
                simplify.finishWithErrors(`${opName}-CreateDistribution`, error)
            })
        })
    })
} catch (err) {
