...
try {
    ...
    provider.setConfig(config).then(function () {
        simplify.uploadLocalFile({
            ...
            ...{ bucketKey: config.Bucket.Key, inputLocalFile: path.join(__dirname, "schedule-rule.yaml") }
            ...
            function processStackData(stackData) {
                ...
                stackData.Outputs.map(function (o) {
                    if (o.OutputKey == `ScheduleEventARN`) {
                        outputData.ScheduleEventARN = o.OutputValue
                    } else if (o.OutputKey == `Region`) {
                        outputData.ScheduleEventRegion = o.OutputValue
                    } else if (o.OutputKey == `StackId`) {
                        outputData.ScheduleEventStackId = o.OutputValue
                    }
                })
            ...
            simplify.createOrUpdateStackOnComplete({
                ...
                simplify.finishWithErrors(`${opName}-CreateScheduler`, error)
            })
        })
    })
} catch (err) {
